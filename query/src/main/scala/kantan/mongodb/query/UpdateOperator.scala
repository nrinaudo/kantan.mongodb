...
object UpdateOperator {
  ...
  final case class Modifiers[A](values: Seq[A], slice: Option[Int], position: Option[Int],
  ...
  implicit def modifiersEncoder[A: BsonValueEncoder]: BsonDocumentEncoder[Modifiers[A]] = {
    ...
    BsonDocumentEncoder.from { case Modifiers(values, slice, position, sort) ⇒ BsonDocument(
